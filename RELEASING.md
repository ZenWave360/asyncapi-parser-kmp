...
- Maven Central: `io.zenwave360.jsonrefparser:asyncapi-parser-kmp`, `-jvm`,
  `-js`
- npm (optional): `@zenwave360/asyncapi-parser-kmp`
...
3. The workflow validates everything, creates the release commit and tag
   `v<VERSION>` on `main`, builds and tests (JVM, JS) without any
   credentials, then waits.
