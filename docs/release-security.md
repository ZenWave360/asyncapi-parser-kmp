...
This document describes how asyncapi-parser-kmp releases are produced, which
security boundaries protect the publication credentials (Maven Central **and**
npm), the required GitHub/npm configuration, and the residual risks. It
follows the same model as `ZenWave360/json-schema-ref-parser-kmp` (the
reference implementation — see that repository's `docs/release-security.md`),
...
3. The workflow automatically: validates → creates the release commit and tag
   → builds and tests (JVM + JS) → waits for approval of the
   'maven-central-upload' environment → signs and uploads the Central
...
| `build-and-test` | Yes — full `./gradlew build` + local staging + `npm pack` (Gradle plugins and Kotlin/JS tooling are arbitrary code execution) | `GITHUB_TOKEN` read-only; **no** Central/npm/signing secrets |
...
and stages the exact Maven publications with
`publishAllPublicationsToLocalStagingRepository` into a local `file://`
repository — no credentials, no signing (the build only signs when signing
credentials are configured, and none are present). It also packs the
Kotlin/JS-generated npm package (`npm pack --ignore-scripts`) after verifying
its `name` and `version`. One artifact is uploaded:

```text
release-staging/
  maven/io/zenwave360/jsonrefparser/...   # unsigned Maven publications
  npm/zenwave360-asyncapi-parser-kmp-<V>.tgz
  SHA256SUMS
...
Same as json-schema-ref-parser-kmp: the bundle is uploaded via the Central
Portal Publisher API with `publishingType=USER_MANAGED` (the API equivalent of
...
1. Create the npm account and the `zenwave360` organization (the package is
   scoped: `@zenwave360/asyncapi-parser-kmp`). Enable 2FA.
2. Build the tarball from the release tag — never from a working tree:
   `git checkout vX.Y.Z && ./gradlew build && npm pack --ignore-scripts ./build/js/packages/asyncapi-parser-kmp`
...
4. On npmjs.com → package → Settings → **Trusted Publisher**: GitHub Actions,
   organization `ZenWave360`, repository `asyncapi-parser-kmp`, workflow
   `release-from-notes.yml`, environment `npm-publish`.
...
Identical model to json-schema-ref-parser-kmp:
...
Same procedures as json-schema-ref-parser-kmp:
...
6. **Shared Central namespace with `json-schema-ref-parser-kmp`** — both
   repositories publish under `io.zenwave360.jsonrefparser`. A compromise of
   either repository's Central credentials risks the other's artifact
   namespace; keep the two repositories' `CENTRAL_TOKEN`/`SIGN_KEY` values
   distinct so revoking one does not require touching the other.
...
- Dependabot (`.github/dependabot.yml`): github-actions SHA pins and Gradle
  dependencies.
